...
*Instruction Set Architecture (ISA)* - the contract/interface between hardware and software
  ...
  - Gray areas exist, x86 is so dominant that it can get around nightmare-ish decode logic. Legacy ISAs still exist because the cost of migrating to newer ISAs outweighs the benefits.
  ...
  - RISC vs CISC - RISC allows for simpler hardware implementations (generally), CISC can have better code density. 
    - Modern CISC processors (e.g. x86) use dynamic translation to generate RISC-like $mu$-ops anyways, so this debate is kinda pointless today.
...
=== Forwarding
  - also called data bypassing, forwarding the result of a previous instruction to an earlier stage. 
    - ignore values read from register file - they contain values from a previous architectural state
...
== Cache - Virtual Memory Interactions

*Quick Review of Virtual Memory*

- *Virtual Memory* - "virtualize" memory, gives the illusion of a larger continuous logical address space.
  - only a small part of the program needs to be in (physical) memory at a given time in a program.
  - We can have synonyms, virtual addresses of different addresses mapping to the same physical address, allowing for better utilization of memory for shared resources like libraries.
  - Allows for efficient process creation. 

- Paging vs. Segmentation: paging involves dividing the logical address space into fixed-size pages, whereas segmentation uses variable-sized segments which represents a part of the program (e.g. call stack)

- Advantages of Virtual Memory
  - *Translation*: programmer gets a consistent, continuous view of memory.
  - Only the most important part of the program is loaded into physical memory.
  *Protection*: different processes have different address spaces, and are thus protected from each other.
    - Different pages can have different protections (some can only be accessed by supervisor status, etc.)
...
- Virtual Address consists of ```sv {VPN, offset}```, Physical Address consists of ```sv {PPN, offset}```.
  - First we need to find the physical address of the page table entry (PTE)
    - $text("PhysAddr of PTE") = "VPN" times "Size of PTE" + "PTBR (Page Table Base Register)"$
  - Then we issue the memory request to get the PTE, which contains our PPN.
  - Concatenate PPN with offset (same offset in Vmem and Pmem) to determine phyical address.

- A one-to-one mapping of PPN $arrow.l.r.double$ VPN is sometimes not possible (not enough space in RAM to store all mappings)
  - Solution: Multi-level page tables. Each virtual address now indexes ```sv {PT1, PT2, ..., offset}```
  - Worst case scenario, a process uses up its entire virtual address space and you have to store extra info for the L1, L2, ..., mappings.
  - More realistically, a process uses its virtual address space rather sparingly, and the multi-level page table saves you from having to allocate large continuous memory chunks that end up not being used. 
    - Allows for fragmentation of the page table in physical memory.
  - Major Downside: now you need extra memory "traversals" just to find a singular PPN.
...
- To avoid aliasing/homonym problem, we physically tag addresses (VIPT/PIPT)
  - while PIPT avoids homonyms, its also suffers from slow memory lookups as it needs to go through address translation each time.

- Nuance #1: *Synonym Problem isn't inevitable for VIPT caches*
  - If $"# sets" <= frac("Page Size", "TLB Cacheline Size")$, then we're guaranteed to not have the synonym problem.
    - Virtual Address can be broken up into ```sv {VPN, offset}```
    - When we're addressing into the TLB, our address is broken up into ```sv {TLB tag, TLB index, block offset}```
    - Two different virtual addresses which alias to the same physical address *will* share page offsets.
    - If TLB index and block offset can be contained s.t. its less than or equal to the size of the page offset, our indexing scheme never "touches" VPN
    - Thus when we have two virtual addresses which alias and are inputs to the TLB, they will index into the same entry, and the tag check will be done with respect to physical addresses.

  - *Case Study: MIPS R10000*
    - Contains L1 (VIPT) and a direct-mapped L2 TLB (PIPT) which is inclusive of the L1
    - Indexing "bleeds" into VPN, thus causing aliasing in L1 TLB.
    - The segment which "bleeds" into VPN is appended to tag in the L2 TLB. Note: tag entry of L2 contains physical address tag + virtual address bits from VPN
    - Assume two addresses A (which already has an entry in L1/L2) and B which is a synonym of A.
    - B indexes into L1 TLB, goes to a different index in L1, cache miss.
    - B is translated into a physical address, goes to L2 cache, goes to same block as A, but fails because the lower bits of the VPNs don't match.
    - This triggers a conflict miss, A's entry in L1 is ejected and replaced with L2.
    - We eliminate the synonym problem as only one of these synonyms have a mapping in L1 at any given point in time.

- Nuance #2: *What do we do on a TLB miss?*
  - We have the option to handle TLB misses in hardware (x86 does this), or software by raising a TRAP and having the OS take care of it.
...
- In an out-of-order processor, instructions sit in Issue Queues/Reservation Stations, waiting for CDB (common data bus) to broadcast speculative register values.
  - Once all operands of an instruction are ready, it can be steered for execution.
  - Multiple instructions in a reservation station can be available at once, and so its advantageous to have a good scheduling policy to select which instruction to select.
    - We can prioritize "older instructions" (age-ordered queue), or "problematic" instructions (instructions with a high degree of dependency)
- Aside: "waking up" and selecting instructions for execution in the reservation stations/issue queues involves a lot of CAM matching, which often lies on the critical path. Thus, its often advantageous to separate them out into separate pipeline stages.

== Tomasulo's Algorithm (with ROB)
\
*Core Idea:*
  - Issue an instruction in the instruction queue to its RS if theres an available slot. Destination is renamed to enqueued ROB ID.
    - Operands read from either register file or ROB
  - When operands are ready, steer the instruction for execution. The results of the instruction are written on the CDB;
    - If not ready, snoop CDB. If CDB broadcasts the renamed id (ROB ID) that happens to the operand, write that value into the RS entry, mark that operand as ready.
  - After being executed, as part of the CDB broadcast, broadcast all results to all RS and write the speculative value into ROB.
    - Mark the old RS entry as available.
  - When instruction reaches the top of the ROB head, dequeue it, write the new register value into register file.

- *Pros:* It does what it needs to do--eliminates WAW, WAR dependencies through register renaming
- *Cons:* Result of functional units stored in RS, ROB, and Register File simultaneously (inefficient)

- These problems associated with Tomasulo's Algorithm are often solved through *ERR (Explicit Register Rename)* and register file read-after-issue.
  - Would go in-depth about ERR, but this is just the basis of mp ooo 

== Precise Exceptions

- *Exceptions* - internal problems in the execution of a program.
  - Ex: divide by zero, access protections, page fault (sometimes), etc.
- *Interrupts* - external events in the execution of a program.
  - I/O devices, power failure, etc.

*Both require:*
  -  the processor to halt (sometimes temporarily)
  - saving processor architectural state
  - handling the exception/interrupt (you need some handler)

*When do we handle them?:*
  - We only handle exceptions when its known (and known to be not speculative)
  - Interrupts: it depends, sometimes we don't need to handle it right away if its low-priority

- *Precise Exceptions*: The architectural state should be consistent when the exception is ready to be handled.
  - All previous instructions should be committed, all following instructions shouldn't 

*Why do we want precise exceptions?*
  - Semantics
  - Allows for a better debugging experience as a programmer (its easier to determine what caused the exception)
  - Allows easy recovery from the exception
  - Enables traps into software
    - Ex: Software-defined opcodes: undefined opcode exception goes into a custom handler/subroutine.
    
= Memory Systems 

== DRAM 

- *Dynamic Random Access Memory* - uses capacitors to store data.
  - "Dynamic" means you need you refresh the DRAM cell every couple ms or so.
  - Destructive reads, as the capacitor discharges.
  - Capacitors are small, and thus $Q = C V$, the charges are small as well.
    - You need sense amplification to read well.

- Organized in a hierarchy: Channel #sym.arrow DIMM #sym.arrow Rank #sym.arrow Chip #sym.arrow Bank #sym.arrow Row #sym.arrow Column/Cells
  - DRAM reads read an entire row into a row buffer, which then goes through a column decoder to return the specific sub-portion of the row.

==== DRAM Policies

*Three DRAM operations*
  - ACT: open a row
  - CAS: perform R/W operation on the opened row (column access strobe)
  - PRE: close the row and precharge
...
*Scheduling Policies*
  - FCFS: naive, retrieve whatever address you get first.
  - FRFCFS: look at FIFO, if you have a row hit, retrieve that data out-of-order
    - good for MLP, temporal locality

== Load-Store Queue (LSQ)

- The most basic implementation of a LSQ is a in-order FIFO
  - Memory operations are issued only when it hits the top of ROB
  - Simple and requires no "recovery/rollback" logic, but doesn't exploit ILP, MLP, and kinda bad for performance.

- Store-to-load forwarding: Check LSQ for older stores to the same address, forward store data and execute loads out-of-order.

*Split-LSQ and Post-Commit Store Buffers*

- Separate load and store queues allow better forwarding within a core, drains to cache on retirement.
- On commit, loads can be moved to a post-commit store buffer, where they are globally visible, and eventually write to L1D cache.
  - Decouples commit from write-port availability
  - Enables memory coalescing into a single cache line (imagine ```c memcpy``` and ```c memset```)
  - Allows cache to busy with loads
  - Allows store queue to drain faster (less backpressure)

- Aside: ```asm FENCE``` instructions drain the LSQ, preventing newer memory instructions from entering the LSQ until its empty.
  - ```asm FENCE.I``` is a thing in RISC-V which also flushes L1I, allowing for things like self-modifying code.

- *Aggressive memory re-ordering and forwarding has an associated power/area cost.*

#pagebreak()

= Multicore Systems and Multithreading
\
*Why can't we just increase issue width, pipeline depth, and clock frequency?*

#figure(
  image("hamster-meme-goofy-hamster.gif", width: 60%),
  caption: [Them hardware engineers in Austin--*iykyk*],
)

We can derive a rough function for the optimal number of pipeline stages in a processor ($P_"opt"$)

$

P_"opt" = sqrt(frac(N_I t_p, alpha gamma N_H t_D))

$

where: $N_I$ is number of instructions, $t_D$ is the latch delay, $N_H$ is the number of hazards, $alpha$ is average degree of superscalar processing, $gamma$ is a factor which represents the average "impact" of a hazard. 

This formula helps identify tradeoffs:
  - e.g. Better branch prediction -> less hazards -> we can increase pipeline depth
  - Larger superscalar fetch -> $alpha$ increases -> we need to decrease pipeline depth
  - *This formula does not take into account power*

$

P_"dyn" = n C V^2 f alpha

$

In this case, $alpha$ is the activity factor. We're also assuming $P_"static" << P_"dyn"$
  - Factoring this into the equation, we get an ugly *polynomial*
  - In other words, power will blow up the moment you do deep pipelines and push your processor to higher and higher clock frequencies.

Imagine a system with N transistors
  - If it were all dedicated towards a single core, you have $O(N)$ power and area, and $O(sqrt(N))$ performance
  - If it were dedicated towards $k$ multi-cores, then you have $k * times O(N/k) = O(N)$ total power and area, and $O(k times sqrt(N/k)) = O(sqrt(k) sqrt(N))$ performance.

== Multi-Core Systems
\
*Mandatory Amdahl's Law Mention* - It always comes up
...
where f is the % of the program which is parallelizable, and p is the number of parallel processors.

Gustafson's Law - You can scale up the size of a parallelizable program with respect to the number of parallel processors.

*Shared vs. Distributed Memory*
  - Shared memory interfaces are great from a programmer perspective, but introduces race conditions between threads, and less "safety" memory-wise.
  - Distributed Memory is less convenient to work with, but requires explicit messaging/communication between threads.

*Interconnect Topologies*
  - Shared bus and arbitration (low bandwidth, low area/power)
  - Crossbar connection (high bandwidth, high area/power)
  - Mesh networks, NoC-based communication, etc.

*Cache Coherence*
  - How do you ensure each core gets the most recent data?
  - How do memory load/stores propagate to other cores?
  - Very important!!!!

== Multithreading
  - Instead of multiple cores, you can support a single core with multiple threads of execution
    - A thread is just a PC, associated regfile/architectural state
